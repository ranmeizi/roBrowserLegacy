...
function createConfigJS() {
	const start = Date.now();
	const pwaDir = './applications/pwa';
	const outDir = dist + platform;
	const configSrc = pwaDir + '/Config.js';
	const configLocalSrc = pwaDir + '/Config.local.js';
	const configDest = outDir + '/Config.js';
	const configLocalDest = outDir + '/Config.local.js';

	fs.copyFileSync(configSrc, configDest);

	if (fs.existsSync(configLocalSrc)) {
		fs.copyFileSync(configLocalSrc, configLocalDest);
	} else if (fs.existsSync(configLocalDest)) {
		fs.rmSync(configLocalDest);
	}

	console.log('Config.js copied from applications/pwa in', Date.now() - start, 'ms.');
}
